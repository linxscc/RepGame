...
namespace GameLogic
{
    public class HandleNetworkRequest
    {
        ...
        // 处理出牌请求
        private void HandlePlayCards(NetPeer peer)
        {
            // 停止该玩家的回合计时器
            ...
            try
            {
                ...
                pendingCardData.Remove(peer);
                // 反序列化卡牌列表
                var clientCards = CardModel.DeserializeList(cardsJson);

                // 验证卡牌是否在玩家手牌中，并从玩家手牌中获取完整的卡牌数据
                var validatedCards = cardManager.ValidateAndRemovePlayedCards(peer.Id, clientCards);

                if (validatedCards == null)
                {
                    // 发送错误响应
                    var errorResponse = ApiResponse<DamageResult>.Fail(400, "无效的卡牌，这些卡牌不在您的手牌中");
                    var errorWriter = new NetDataWriter();
                    errorWriter.Put("DamageResult");
                    errorWriter.Put(errorResponse.Serialize());
                    peer.Send(errorWriter, DeliveryMethod.ReliableOrdered);
                    Debug.LogError($"[SERVER] Player {peer.Id} tried to play invalid cards");
                    return;
                }

                // 使用DamageCalculator计算伤害，同时指定伤害类型为攻击者
                var damageResult = damageCalculator.CalculateDamage(validatedCards, DamageType.Attacker);

                // 从共享牌库中抽取三张随机卡牌
                List<CardModel> newCards = cardManager.DrawRandomCardsFromSharedDeck(peer.Id, 3);

                // 将抽取的新卡牌添加到伤害结果中
                damageResult.newCards = newCards;
                ...
                // 获取对手玩家信息
                int? receiverId = cardManager.GetOpponentId(peer.Id);
                NetPeer receiverPeer = receiverId.HasValue ? netManager.GetPeerById(receiverId.Value) : null;
                ...
                if (receiverPeer != null)
                {
                    ...
                    receiverPeer.Send(receiverWriter, DeliveryMethod.ReliableOrdered);

                    // 处理伤害计算并检查游戏是否结束
                    bool isGameOver = cardManager.ProcessDamage(peer.Id, receiverId.Value, damageResult);

                    // 如果游戏结束，不再切换回合
                    if (isGameOver)
                    {
                        cardManager.CleanRooms(peer.Id);
                        Debug.Log($"[SERVER] Game over: Player {peer.Id} wins against Player {receiverId}");
                        return;
                    }
                }

                // 获取房间内所有玩家
                List<NetPeer> roomPeers = cardManager.GetRoomPeers(peer.Id);

                // 获取下一个玩家（对手）
                NetPeer nextPlayer = cardManager.GetNextPlayer(peer.Id);

                // 如果找到下一个玩家，发送回合通知
                if (nextPlayer != null && roomPeers.Count > 0)
                {
        ...
        public void HandleCompCards(NetPeer peer, string requestData)
        {
            try
            {
                ...
                var clientRequest = JsonUtility.FromJson<ApiResponse<List<CardModel>>>(requestData);                // 将客户端的卡牌数据转换为服务器端的CardModel
                var cardsToCompose = clientRequest.Data;

                // 验证卡牌并获取完整数据
                var validatedCompCards = cardManager.ValidateAndRemovePlayedCards(peer.Id, cardsToCompose);

                if (validatedCompCards == null)
                {
                    SendCompErrorResponse(peer, "无效的卡牌，这些卡牌不在您的手牌中");
                    return;
                }

                // 使用CardManager处理卡牌合成逻辑
                var newCards = cardManager.HandleCardComposition(peer.Id, validatedCompCards);                // 创建合成结果响应
                var compResult = new ApiResponse<CompResult>
                {
                    ...
                    {
                        Success = true,
                        UsedCards = validatedCompCards,  // 使用完整卡牌数据
                        NewCards = newCards,
